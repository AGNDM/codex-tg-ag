...
func TestPolicyUsesNativeAgentsWithModelNeutralLead(t *testing.T) {
	document := Document()
	for _, want := range []string{"small Azure Linux server", "Codex App Server", "real Codex Project", "luna_executor", "gpt-5.6-sol", "astra_advisor", "gpt-6-astra", "any model currently available", "two serious attempts", "Critical path"} {
	...
	if strings.Contains(reminder, "you are the Sol lead") {
		t.Fatalf("runtime reminder still forces Sol: %s", reminder)
	}
