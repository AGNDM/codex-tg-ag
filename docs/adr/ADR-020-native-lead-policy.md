...
The daemon embeds a versioned `lead-default` policy, records `policy_id` and `policy_version` for each durable lead, and exposes `/agent policy` plus `/agent policy apply`. Applying a policy starts a normal turn in the lead's persistent App Server thread. If an outdated lead receives a normal task first, the daemon injects the full policy with that task and records the upgrade after App Server accepts the turn. Current-policy lead turns normally receive a compact reminder so the critical delegation and approval rules survive long context and compaction.

A Project may declare `Codex-TG Project Agent Policy: project-v1` as an exact
...
- Project `AGENTS.md` continues to define project-specific work rules. A Project
  that explicitly adopts `project-v1` also carries the shared Agent policy;
  Lead-only responsibility remains conditional on the runtime role marker.
- Applying a policy consumes one normal lead turn because the instruction becomes part of the durable thread history.
- Adopted Projects avoid appending the full reminder to every Telegram user
  message. Project instructions still consume model context, but do not appear
  as repeated operator-message text.
