...
- deferred UI state is not authoritative input state; an explicit armed, reply, or current-turn target may attempt `turn/steer` once and lets App Server decide whether that exact turn is still active
- a steer error may fall back to `turn/start` only when App Server explicitly reports no active turn and a successful re-read confirms that no newer active turn exists; timeouts, unknown errors, and failed re-reads never start a parallel turn
- callback Steer remains bound to its original panel, message, and turn; callback Stop remains bound to its original turn; neither old button may target a newer active turn
...
- Tests must cover both parts of the contract:
  ...
  - a latest-turn steer may succeed during the defer window without opening a new turn
  - stale Stop/Steer callbacks cannot target a newer active turn
