...
- 2026-09-14 AEST Azure Lead Policy deployment installed the checked-in Codex multi-agent configuration and the `luna_executor` / `astra_advisor` standalone agent files for the service user, restarted the daemon, and injected `lead-default` version 1 into all three persistent Lead threads before recording the applied version in SQLite. A normal persistent `codex exec` smoke used a Sol lead to delegate exactly one tool-free arithmetic task to native `luna_executor`, wait, and independently verify the correct result. An earlier `--ephemeral` smoke documented a Codex 0.154.0 boundary: ephemeral exec threads are not registered with the collaboration router and fail native spawn with `no thread with id`. Ubuntu's maintained `bwrap-userns-restrict` AppArmor profile was then explicitly approved and enabled while the global unprivileged-user-namespace restriction stayed active. A direct namespace smoke passed, followed by a persistent Sol-to-Luna delegation in which Luna ran a read-only project-root listing and the Lead independently repeated the shell command with matching output. No private Project, thread, topic, chat, or message ids are recorded here.
