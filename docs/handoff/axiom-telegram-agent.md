...
The operator communicates with Axiom, not directly with worker subagents. Keep the Lead on `gpt-5.6-sol` with `medium` reasoning unless the operator explicitly changes it. Use the native `luna_executor` for bounded execution and the read-only `astra_advisor` for the escalation conditions in `lead-default` policy.
...
4. `docs/adr/ADR-020-native-lead-policy.md`
...
- `lead-default` v2 describes delegation, escalation, critical-path approval, the Azure environment, Project context, and small-server resource constraints.
