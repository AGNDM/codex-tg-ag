...
Validate both layers after loading the profile: first run a minimal read-only `bwrap --unshare-user` command, then ask a persistent Sol lead to delegate one read-only shell task to `luna_executor`, wait, and independently verify the output. Restart the Telegram daemon afterward so the production App Server starts against the verified host policy.
